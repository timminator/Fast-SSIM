...
#if defined(__AVX2__)
    #include <immintrin.h>
    #define SIMD_NAMESPACE avx2_impl
#else
    #define SIMD_NAMESPACE sse_impl
#endif
...
static unsigned int GetTargetThreadCount(int total_rows, int block_size) {
    ...
    unsigned int useful_threads = (total_rows + block_size - 1) / block_size;
...
// --- NAMESPACE WRAPPER ENDPOINTS ---
double MSE_Byte_Impl(Byte* pDataX, Byte* pDataY, int step, int width, int height) {
    return MSE_Data_Byte(pDataX, pDataY, step, width, height);
}
double MSE_Float_Impl(float* pDataX, float* pDataY, int step, int width, int height) {
    return MSE_Data_t<float, float>(pDataX, pDataY, step, width, height);
}
float PSNR_Byte_Impl(Byte* pDataX, Byte* pDataY, int step, int width, int height, int maxVal) {
    double mse = MSE_Data_Byte(pDataX, pDataY, step, width, height);
    mse = MAX(mse, 1e-10);
    return (float)(10*log10((double)maxVal*maxVal/mse));
}
float PSNR_Float_Impl(float* pDataX, float* pDataY, int step, int width, int height, double maxVal) {
    double mse = MSE_Data_t<float, float>(pDataX, pDataY, step, width, height);
    mse = MAX(mse, 1e-10);
    return (float)(10*log10(maxVal*maxVal/mse));
}
float SSIM_Byte_Impl(Byte* pDataX, Byte* pDataY, int widthBytes, int width, int height, int win_size, int maxVal) {
    return SSIM_Generic<Byte>(pDataX, pDataY, widthBytes, width, height, win_size, (double)maxVal);
}
float SSIM_Float_Impl(float* pDataX, float* pDataY, int step, int width, int height, int win_size, double maxVal) {
    return SSIM_Generic<float>(pDataX, pDataY, step, width, height, win_size, maxVal);
}
float SSIM_Byte_Slow_Impl(Byte* pDataX, Byte* pDataY, int widthBytes, int width, int height, int win_size) {
